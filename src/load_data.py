...
from loren_frank_data_processing.core import get_data_structure, reconstruct_time
...
def get_track_segments(epoch_key, animals):
    ...
    """
    environment = np.asarray(make_epochs_dataframe(animals).loc[epoch_key].environment)[
        0
    ]

    coordinate_path = os.path.join(
        animals[epoch_key[0]].directory, f"{ENVIRONMENTS[environment]}_coordinates.mat"
    )
...
def get_adhoc_ripple(epoch_key, tetrode_info, position_time, position_to_linearize):
    ...
    ripple_filtered_lfps = pd.DataFrame(
        filter_ripple_band(np.asarray(ripple_lfps)), index=ripple_lfps.index
    )
    ...
    ripple_consensus_trace = pd.DataFrame(
        get_ripple_consensus_trace(ripple_filtered_lfps, LFP_SAMPLING_FREQUENCY),
    ...
    new_index = pd.Index(
        np.unique(np.concatenate((ripple_consensus_trace.index, position_time))),
        name="time",
    )
    ripple_consensus_trace = (
        ripple_consensus_trace.reindex(index=new_index)
        .interpolate(method=interpolation_method)
        .reindex(index=position_time)
    )
    ripple_consensus_trace_zscore = zscore(ripple_consensus_trace, nan_policy="omit")

    instantaneous_ripple_power = np.full_like(ripple_filtered_lfps, np.nan)
    not_null = np.all(pd.notnull(ripple_filtered_lfps), axis=1)
    instantaneous_ripple_power[not_null] = (
        get_envelope(np.asarray(ripple_filtered_lfps)[not_null]) ** 2
    )
    instantaneous_ripple_power_change = np.nanmedian(
        instantaneous_ripple_power / np.nanmean(instantaneous_ripple_power, axis=0),
        axis=1,
    )
...
def get_adhoc_multiunit(
    position_info, tetrode_keys, time_function, position_to_linearize
):
    time = position_info.index
    multiunits = get_all_multiunit_indicators(tetrode_keys, ANIMALS, time_function)
    multiunit_spikes = (np.any(~np.isnan(multiunits.values), axis=1)).astype(np.float)
...
def load_data(
    ...
    try:
        neuron_info = make_neuron_dataframe(
            ANIMALS, exclude_animals=["Monty", "Peanut"]
    ...
    theta = get_theta(tetrode_info, position_info)
...
def _get_pos_dataframe(epoch_key, animals):
    ...
    return pd.DataFrame(position_data[:, 1:], columns=field_names[1:], index=time)
...
def _get_linear_position_hmm(
    ...
    position_df = pd.concat(
        (position_df, linearized_position_df.set_index(position_df.index)), axis=1
    )
    try:
...
def get_sleep_and_prev_run_epochs():
    ...
    for _, df in epoch_info.groupby(["animal", "day"]):
        ...
        is_sleep_after_run = np.asarray((df.iloc[1:].type == "sleep") & is_w_track)
...
def get_gnd_tetrode_filename(tetrode_key, animals):
    ...
    filename = (
        f"{animals[animal].short_name}eeggnd{day:02d}"
...
def get_theta(tetrode_info, position_info):
    ...
    new_index = pd.Index(
        np.unique(np.concatenate((position_info.index, gnd_lfp.index))), name="time"
    )
    ...
    theta_power_zscore = (
        theta_power - np.nanmean(theta_power[speed >= 4])
    ) / np.nanstd(theta_power[speed >= 4])
